...
public class CharacterLocomotion : CharacterComponent
{
	...
	public float m_MinCrouchedSpeed = 1.5f;
	public float m_MaxCrouchedSpeed = 4.0f;
	...
	[Header("Input")]
	public float m_TurnRate = 500.0f;
	...
	private void HandleSprintInput() {
		if (sprintInput != OwnerCharacter.Input.Sprint) {
			sprintInput = OwnerCharacter.Input.Sprint;
		}
	}

	private void HandleCrouchInput() {
		if (crouchInput != OwnerCharacter.Input.Crouch) {
			crouchInput = OwnerCharacter.Input.Crouch;
		}
	}
	...
	private void FixedUpdate() {
		...
		if (moveInput) {
			if (sprintInput) {
				targetPlanarVelocity = wsInputDirection * m_SprintSpeed;
			}
			else {
