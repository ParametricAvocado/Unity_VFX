	...
	Properties
	{
		...
		SubShader
		{
			...
			Pass
			{
				...
				#pragma vertex vertLightshaftsUnlit
				...
				//Local Space light direction (for Directional Lights) or position (for Point Lights)
				float4 _LightCoords;
				float _LightAtten;

				void vertLightshaftsUnlit(appdata_particles v, out VertexOutput o)
				{
					float3 lightDir = lerp(_LightCoords.xyz, normalize(v.vertex - _LightCoords.xyz) , _LightCoords.w);

					float NdotL = dot(v.normal, lightDir);
					v.color.a *= saturate(NdotL)*_LightAtten;
